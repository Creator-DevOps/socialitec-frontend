// src/hooks/useShowPreviewReport.ts
...
export const useShowPreviewReport = () => {
  const { token } = useAuth();
  ...
  const openPreview = useCallback(
    async (document_id: number, filePath: string) => {
      ...
      try {
        ...
        // 4) Crear URL de blob y mostrar en un iframe
        ...
        win.document.write(
          `<html><head><title>Vista previa</title>
           ...
           <body>
             <iframe src="${blobUrl}" style="border:none;width:100%;height:100%;"></iframe>
           </body></html>`
        );

        // 5) Limpiar URL transcurrido un minuto
