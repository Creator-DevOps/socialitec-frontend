...
import DocumentIcon from "@icons/menu/templates.svg";
...
const TemplateCard = ({ item }: Props) => {
  const { openEdit, openDelete, openShow,onDownload} = useTemplates();
  return (
    <div className="flex flex-col justify-between p-3 border border-gray-100 shadow-md w-30 h-40 md:w-35 md:h-45 bg-white hover:bg-gray-50 transition">
      <div className="flex flex-col h-[90%]">
        ...
        <div className="h-1/3 flex items-center justify-center">
          <p className="text-gray-500 text-center line-clamp-3 leading-tight !text-[10px] max-h-10">{item.description}</p>
        </div>
      </div>
